#!/usr/bin/env python3
"""Build and verify the exact minimal .NET runtime used by G4 matches.
...
KIND = "omega-nnue-king-state-v4-dotnet-runtime-bundle"
REPO = Path(__file__).resolve().parents[2]
WORKSPACE = REPO.parent
RUNTIME_ROOT = (
    REPO / "tools/omega_nnue/frozen_runtime/king-state-v4/dotnet-runtime"
)
MANIFEST_PATH = (
    REPO
    / "tools/omega_nnue/frozen_runtime/king-state-v4/dotnet-runtime.manifest.json"
)
...
def self_test() -> None:
    verify_manifest()
    with tempfile.TemporaryDirectory(prefix="omega-g4-dotnet-runtime-") as directory:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "build":
        ...
        self_test()
        print("Generation-4 .NET runtime self-test passed")
    return 0
