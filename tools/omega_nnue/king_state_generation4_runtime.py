#!/usr/bin/env python3
"""Freeze and verify the Python/NumPy runtime used by Generation 4.

The runtime seal is created before any Generation-4 teacher label.  Rules-only
...
SCHEMA_VERSION = 1
KIND = "omega-nnue-king-state-v4-python-runtime"
PROFILE_ID = "king-state-v4-omega-decision-v1"
STATUS = "frozen-before-generation-4-teacher-labels"
...
def _require_environment() -> None:
    if NUMPY_WAS_PRELOADED:
        raise ValueError("NumPy was imported before the Generation-4 runtime contract")
...
def _payload(*, created_utc: str | None = None) -> dict[str, Any]:
    return {
        ...
        "informationBoundary": {
            "generation4SourcePositionFieldsDecoded": 0,
...
def _create(path: Path) -> None:
    ...
    if present:
        raise ValueError(
            "Generation-4 teacher/training artifacts predate the runtime freeze: "
            + ", ".join(present)
        )
    _atomic_create(resolved, _canonical_json(_payload()))
    verify_manifest(resolved)
    print(
        f"Published Generation-4 Python/NumPy runtime manifest: {resolved} "
...
def _self_test() -> None:
    ...
    print("Generation-4 Python/NumPy runtime self-tests passed.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "create":
        ...
        print(f"Verified Generation-4 Python/NumPy runtime: {args.output.resolve()}")
