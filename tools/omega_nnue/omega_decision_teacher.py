#!/usr/bin/env python3
"""Resumable, fail-closed decision-teacher pipeline for Omega Chess.
...
* select fresh roots from completed HCE-only OmegaMatch trajectories without
  reading their scores;
...
import validate_king_state_v4_preregistration as v4_prereg
...
DATA_PROFILE_ID = "omega-decision-v1"
SOURCE_SEED = 2026072201
ROOT_SELECTION_SEED = 2026072202
SIBLING_EXPLORATION_SEED = 2026072203
COMPONENT_SPLIT_SEED = 2026072204
...
TARGET_OPAQUE_DECLARATION_KEYS = frozenset(
    {"targetOpaque", "targetOrScoreFieldsDecoded", "targetOrScoreFieldsEmitted"}
)
...
FINAL_FREEZE_KIND = "omega-nnue-king-state-v4-final-freeze-seal"
FINAL_FREEZE_STATUS = "target-blind-generation-4-design-frozen-before-teacher-labels"
PRELABEL_STATUS = "target-opaque-frozen-before-teacher-search"
SEARCH_WORKERS = 4
SOURCE_NODES = 2000
SOURCE_MAX_PLIES = 1
SOURCE_OPENINGS = 6400
SOURCE_OPENINGS_PER_PHASE_SIDE = 800
SOURCE_TRAJECTORY_PAIRS = 8192
REPO = Path(__file__).resolve().parents[2]
SOURCE_OPENING_BUILDER = REPO / "tools/omega_nnue/king_state_generation4_source.py"
...
def _producer_identity() -> dict[str, Any]:
    ...
    return {
        ...
        "generation4PreregistrationValidator": _identity(Path(v4_prereg.__file__)),
...
def _verify_producer_identity(value: Any, description: str) -> dict[str, Any]:
    ...
    producer = _strict_keys(
        value,
        {
            ...
            "omegaNnue",
            "selectScreen",
            "generation4PreregistrationValidator",
            "priorProjection",
            "python",
        },
        description,
...
def _verify_final_freeze(
    ...
    v4_prereg.validate_profile(profile, mode="frozen", verify_external=True)
    ...
    namespaces = _strict_keys(
        ...
        set(v4_prereg.EXPECTED_NAMESPACES),
    ...
    validator_identity = _identity(Path(v4_prereg.__file__))
    ...
    declaration = _strict_keys(
        seal["declaration"],
        {
            ...
            "generation4TeacherTargetsDecoded",
            "generation4ValidationTargetsDecoded",
            "generation4HeldOutTargetsDecoded",
        },
...
def _extract_source_candidates(
    ...
    *,
    ...
    for game_id, attempt_number, attempt in complete_games:
        ...
        group_payload = (
            f"omega-decision-source-group-v1\0{pin['sha256']}\0{run_id}\0"
            f"{source_tag}"
        )
        ...
        for line_number, ply in ordered_plies:
            ...
            root_payload = (
                f"omega-decision-root-v1\0{pin['sha256']}\0{run_id}\0"
                f"{game_id}\0{attempt_number}\0{ply_number}\0{ofen}"
            )
            root_id = hashlib.sha256(root_payload.encode("utf-8")).hexdigest()
            rank = hashlib.sha256(
                f"omega-decision-root-rank-v1\0{seed}\0{root_id}".encode("utf-8")
            ).hexdigest()
            candidates.append(
                {
                    "schemaVersion": 1,
...
def _prepare_roots(args: argparse.Namespace) -> None:
    ...
    if len(args.events) != 1:
        raise ValueError("G4 source provenance requires exactly one pinned events ledger")
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            ...
            if (
                ...
                and TARGET_LIKE_KEY.search(normalized)
...
def _forbidden_extraction_policy() -> dict[str, Any]:
    ...
    return {
        ...
        "generation4NamespacePolicy": "reject-source-path-before-open",
...
def _catalog_sensitive_field_name(value: str) -> bool:
    ...
    return bool(TARGET_LIKE_KEY.search(normalized))


def _assert_prior_catalog_source(path: str | Path) -> Path:
    """Reject all known G4 namespaces before opening an explicit source."""
    ...
    forbidden_components = {
        "omegadecisionv1",
        "kingstatev4",
        "kingstatev4development",
        "buildkingstatev4",
    }
    if (
        any(marker in normalized for marker in FORBIDDEN_G4_PATH_MARKERS)
        or components & forbidden_components
        or path.name.lower().startswith("omega-nnue-king-state-v4-")
    ):
        raise ValueError(f"generation-4 namespace is forbidden as prior input: {path}")
    if not path.is_file():
...
def _extract_catalog_source(
    ...
    for observation in observations:
        observation["sourceInputSha256"] = before["sha256"]
...
def _catalog_position_id(row: dict[str, Any]) -> str:
    basis = {
        ...
        "sourceInputSha256": str(row.get("sourceInputSha256", "")).lower(),
    }
...
def _catalog_rows_from_observations(
    ...
    for observation in observations:
        key = (
            ...
            str(observation.get("sourceInputSha256", "")).lower(),
        )
    ...
    for _, observation in sorted(unique.items()):
        ...
        row: dict[str, Any] = {
            ...
            "sourceInputSha256": observation["sourceInputSha256"],
        }
...
def _verify_forbidden_source_inventory(value: Any) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(value):
        ...
        # Apply the G4 namespace boundary to the declared path before hashing
...
def _verify_source_projection_manifests(
    ...
    return {
        "manifestPins": pins,
        "sourceInputSha256": sorted(transitive_hashes),
...
def _build_forbidden_catalog_from_sources(
    ...
    resolved_sources = [_assert_prior_catalog_source(source) for source in sources]
...
def _load_forbidden_catalogs(
    ...
    for manifest_path in manifests:
        ...
        forbidden["sourceInputSha256"].update(
            projection_evidence["sourceInputSha256"]
        )
        ...
        forbidden["sourceInputSha256"].update(source_hashes)
        ...
        for line_number, row in records:
            ...
            for field, bucket in (
                ...
                value = row.get(field, "")
                if value:
                    if field == "sourceInputSha256" and not HEX_SHA256.fullmatch(
                        value.lower()
                    ):
                        raise ValueError(
                            f"{catalog_path}:{line_number}: malformed source SHA-256"
                        )
                    forbidden[bucket].add(value.lower() if field.endswith("Sha256") else value)
            source_sha = str(row.get("sourceInputSha256", "")).lower()
...
def _build_synthetic_forbidden_catalog(
    directory: Path, rows: Sequence[dict[str, Any]]
) -> tuple[Path, Path]:
...
def _verify_prelabel_seal(
    ...
    for name in ("samplerCompletionSeal", "componentMap"):
    ...
    if (
        ...
        raise ValueError("pre-label policy differs from the frozen G4 contract")
    ...
    if (
        int(coverage["roots"]) != 5120
        or int(coverage["children"]) <= 0
        or int(coverage["sourceGroups"]) != 5120
...
def _choose_four(
    ...
    *,
    ...
    if len(children) < 4:
        raise ValueError("root has fewer than four legal children")
    ranked = sorted(
        children,
        key=lambda child: (
    ...
    hard_pool.sort(
        key=lambda child: hashlib.sha256(
            f"omega-hard-negative-v1\0{seed}\0{root_id}\0{_item_id(child)}".encode(
                "utf-8"
            )
        ).hexdigest()
    )
...
def _select(args: argparse.Namespace) -> None:
    ...
    incomplete_roots = {
        ...
        if len(items) < 4 or any(_item_id(item) not in shallow for item in items)
    }
    rejected_groups = {
        str(item["groupId"])
        ...
        for side in ("w", "b"):
            ...
            for ordinal, (root_id, _) in enumerate(
                ...
                chosen_root_roles[root_id] = (
                    "primary" if ordinal < primary_per_side else "reserve"
                )
    selected: list[dict[str, Any]] = []
...
def _component_splits(
    ...
    *,
    ...
    for component in sorted(set(components.values())):
        draw = int(
            hashlib.sha256(
                f"omega-decision-split-v1\0{seed}\0{component}".encode("utf-8")
            ).hexdigest()[:16],
            16,
        ) % 1_000_000 / 10_000.0
        split = (
...
def _prelabel_freeze(args: argparse.Namespace) -> None:
    ...
    for root in roots:
        ...
        prior_exact = exact_owner.setdefault(exact, group)
        if prior_exact != group:
            raise ValueError("exact root input crosses source opening groups")
        for signature in signatures:
            prior = signature_owner.setdefault(signature, group)
            if prior != group:
                raise ValueError("conservative root orbit crosses source opening groups")
    ...
    forbidden, forbidden_pins = _load_forbidden_catalogs(
        [_resolve(path) for path in args.forbidden_position_manifest]
    )
    cli_forbidden = [pin["manifest"] for pin in forbidden_pins]
    if cli_forbidden != _frozen_identity_list(
        prereg, "forbiddenPositionCatalogManifests"
    ):
        raise ValueError("CLI forbidden manifests differ from finalFreezeIdentities")
    source_input_hashes = {
        str(source.get("source", {}).get("sha256", "")).lower()
        for source in root_manifest.get("sources", [])
        if isinstance(source, dict)
    }
    if source_input_hashes & forbidden["sourceInputSha256"]:
        raise ValueError("source telemetry input was used by a prior corpus")
    for root in roots:
        source_game = str(root.get("sourceGameId") or "")
        source_run = str(root.get("sourceRunId") or "")
        if source_game in forbidden["sourceGameIds"] or source_run in forbidden[
            "sourceRunIds"
        ]:
            raise ValueError("source game/run was used by a prior corpus")
    for item in [*roots, *children]:
        ...
        if exact in forbidden["exact"]:
            raise ValueError("exact position/input was used by a prior corpus")
        if set(signatures) & forbidden["signatures"]:
            raise ValueError("conservative orbit was used by a prior corpus")
    ...
    components, split_for_component = _component_splits(
        children,
        ...
        _atomic_json(
            output,
            {
                ...
                "policy": {
                    ...
                    "priorReusePolicy": "abort exact, conservative orbit, source game/run, or source input reuse",
                },
                "coverage": {
                    "roots": len(roots),
...
def _self_test() -> None:
    ...
    first = _choose_four(children, shallow, seed=2026072203)
    second = _choose_four(children, shallow, seed=2026072203)
    ...
    components, splits = _component_splits(
        first, seed=2026072204, train_percent=80.0, validation_percent=10.0
    )
    ...
    print("omega_decision_teacher self-test passed")
...
def _parser() -> argparse.ArgumentParser:
    ...
    roots.add_argument("--roots-per-phase", type=int, default=1024)
    ...
    prelabel.add_argument("--roots-per-phase", type=int, default=1024)
    prelabel.add_argument("--reserve-roots-per-phase", type=int, default=256)
    ...
    select.add_argument("--roots-per-phase", type=int, default=1024)
    select.add_argument("--reserve-roots-per-phase", type=int, default=256)
    ...
    finalize.add_argument("--roots-per-phase", type=int, default=1024)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "build-forbidden-catalog":
        ...
        if (
            args.roots_per_phase <= 0
            or args.reserve_per_phase < 0
        ...
        if (
            ...
            or args.roots_per_phase != 1024
            or args.reserve_roots_per_phase != 256
            ...
            raise ValueError("pre-label freeze differs from the frozen G4 contract")
        ...
        if args.roots_per_phase != 1024:
            raise ValueError("final root quota differs from the frozen G4 contract")
        _finalize(args)
