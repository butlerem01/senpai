#!/usr/bin/env python3
"""Shared, target-blind Generation-5 Omega match protocol support.

This module contains only protocol/runtime/suite mechanics.  In particular it
does not import the Generation-5 trainer and cannot decode a teacher,
validation, or held-out target.  The post-held-out authorization command in
``king_state_match_readiness_generation5.py`` performs that separate step.
"""
...
PROTOCOL_KIND = "omega-nnue-king-state-v5-match-protocol"
...
PROTOCOL_PATH = (
    REPO / "validation" / "omega-nnue-king-state-v5-match-protocol.json"
)
FINAL_PROFILE = (
    REPO / "validation" / "omega-nnue-king-state-v5-preregistration.json"
)
FINAL_FREEZE = (
    REPO / "validation" / "omega-nnue-king-state-v5-freeze.seal.json"
)
...
def _protocol_scalar_type_schema() -> dict[tuple[Any, ...], type | None]:
    ...
    add(
        ...
        ("runtimeProvenance", "sourceMatchMustUseFrozenDotnetBundle"),
        ("runtimeProvenance", "sourceMatchCompletionSealRequired"),
        (
            "runtimeProvenance",
            "sourceMatchCompletionSealRequiresExitCodeZero",
        ),
        (
            "runtimeProvenance",
            "sourceMatchCompletionSealRequiresResumeUsedFalse",
        ),
        (
            "runtimeProvenance",
            "sourceMatchCompletionSealRequiresFinalStageSealTrue",
        ),
        (
            "runtimeProvenance",
            "sourceMatchCompletionSealMustAuthenticateFrozenDotnetBundle",
        ),
...
def validate_protocol(
    ...
    if not _allow_noncanonical_for_self_test and path != resolve(PROTOCOL_PATH):
        raise ValueError(
            "Generation-5 match protocol must be the canonical frozen protocol"
        )
    value = strict_load(path, "Generation-5 match protocol")
    ...
    if set(value) != expected_top:
        raise ValueError("Generation-5 match protocol field inventory changed")
    if (
        value.get("schemaVersion") != SCHEMA_VERSION
        or value.get("kind") != PROTOCOL_KIND
        or value.get("profileId") != PROFILE_ID
        or value.get("status")
        != "target-blind protocol frozen before any generation-5 held-out access"
    ):
        raise ValueError("Generation-5 match protocol envelope changed")
    ...
    if provenance != {
        "sourceMatchFramework": ".NET 10.0.9",
        "sourceMatchDotnetRuntimeBundleSha256": (
            "0ce194480dfb9a58a59c79bf94f19cb2eb571635a00547094a9c8ff28bb5f8f8"
        ),
        "sourceMatchMustUseFrozenDotnetBundle": True,
        "sourceMatchCompletionSealIdentityField": "sourceMatchCompletionSeal",
        "sourceMatchCompletionSealRequired": True,
        "sourceMatchCompletionSealRequiresExitCodeZero": True,
        "sourceMatchCompletionSealRequiresResumeUsedFalse": True,
        "sourceMatchCompletionSealRequiresFinalStageSealTrue": True,
        "sourceMatchCompletionSealMustAuthenticateFrozenDotnetBundle": True,
    ...
    if protocol_path(authority["preregistrationTemplate"], "template") != resolve(
        REPO / "validation/omega-nnue-king-state-v5-preregistration.template.json"
    ...
    expected_runtime_paths = {
        "dotnetHost": "tools/omega_nnue/frozen_runtime/king-state-v5/dotnet-runtime/dotnet.exe",
        "dotnetRuntimeManifest": "tools/omega_nnue/frozen_runtime/king-state-v5/dotnet-runtime.manifest.json",
        "dotnetRuntimeTool": "tools/omega_nnue/king_state_dotnet_runtime_generation5.py",
        "matchCoreSource": "tools/omega_nnue/king_state_matches.py",
        "engine": "tools/omega_nnue/frozen_runtime/king-state-v5/engine/senpai.exe",
        "omegaMatchAssembly": "tools/omega_nnue/frozen_runtime/king-state-v5/omegamatch/OmegaMatch.dll",
        "omegaMatchAppHost": "tools/omega_nnue/frozen_runtime/king-state-v5/omegamatch/OmegaMatch.exe",
        "rootSamplerAssembly": "tools/omega_nnue/frozen_runtime/king-state-v5/root-sampler/OmegaRootSampler.dll",
        "rootSamplerAppHost": "tools/omega_nnue/frozen_runtime/king-state-v5/root-sampler/OmegaRootSampler.exe",
        "rootSamplerRulesAssembly": "tools/omega_nnue/frozen_runtime/king-state-v5/root-sampler/ChessLib.dll",
    }
    ...
    _BOUND_VERIFY_MODULE_BINDING(
        ...
        "Generation-5 .NET runtime verifier",
...
def install_core_profile(protocol: Mapping[str, Any]) -> None:
    """Install the frozen G5 constants into the exercised match core."""
    ...
    core.SOURCE_NAMES = {
        gate: f"king-state-v5-{gate}-rules-only.jsonl" for gate in GATES
    }
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="omega-g5-protocol-") as directory:
        ...
        mutations = (
            ...
            (
                "source-match provenance bundle",
                lambda value: value["runtimeProvenance"].__setitem__(
                    "sourceMatchDotnetRuntimeBundleSha256", "0" * 64
                ),
            ),
            (
                "source-match completion seal identity field",
                lambda value: value["runtimeProvenance"].__setitem__(
                    "sourceMatchCompletionSealIdentityField", "changed"
                ),
            ),
            (
                "source-match no-resume seal requirement",
                lambda value: value["runtimeProvenance"].__setitem__(
                    "sourceMatchCompletionSealRequiresResumeUsedFalse", False
                ),
            ),
            (
...
if __name__ == "__main__":
    self_test()
    print("Generation-5 match protocol self-test passed")
