#!/usr/bin/env python3
"""Ordered, resumable Generation-4 Omega NNUE match orchestrator.
...
import king_state_match_readiness_generation4 as readiness
...
INTENT_KIND = "omega-nnue-king-state-v4-launch-intent"
COMPLETION_KIND = "omega-nnue-king-state-v4-launch-completion"
ASSESSMENT_KIND = "omega-nnue-king-state-v4-gate-assessment"
DECISION_KIND = "omega-nnue-king-state-v4-gate-decision"
...
def _verify_import_bindings(protocol: Mapping[str, Any]) -> None:
    ...
    if _IMPORTED_CONTRACT.FINAL_PROFILE.is_file():
        profile = _IMPORTED_CONTRACT.strict_load(
            ...
            "Generation-4 final preregistration",
        )
    ...
    if own_path != expected_own:
        raise ValueError("Generation-4 orchestrator source path is noncanonical")
    if identities is not None and not _IMPORTED_CONTRACT.same_identity(
        ...
        raise ValueError("Generation-4 orchestrator differs from its frozen pin")
...
def _strict_event_record(raw: bytes, label: str) -> dict[str, Any]:
    ...
    if record_type == "run":
        ...
        if record.get("ProfileId") != PROFILE_ID:
            raise ValueError(f"{label} ProfileId is not the Generation-4 profile")
...
def _attest_idle(args: argparse.Namespace) -> None:
    ...
    _verify_idle(context)
    print(f"Generation-4 equal-time idle attestation: {path}")
...
def _config_path(context: Context, gate: str) -> Path:
    ...
    expected = contract.namespace(context.protocol, "sealed") / f"king-state-v4-{gate}-match.json"
    if path != expected:
...
def _recompute_report(
    ...
    with tempfile.TemporaryDirectory(prefix="omega-g4-assess-") as directory:
        root = Path(directory)
...
def _launch(args: argparse.Namespace) -> None:
    ...
    print(
        f"Generation-4 {gate} {action} completed with fixed pair budget "
...
def _assess(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        _verify_decision(context, gate)
        print(
            f"Generation-4 {gate} terminal decision recovered: "
    ...
    if terminal:
        ...
        _verify_decision(context, gate)
    print(f"Generation-4 {gate} assessment {sequence}: {decision}")
    return value
...
def _self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="omega-g4-match-state-") as directory:
        root = Path(directory)
        events = root / "events.jsonl"
    ...
    with tempfile.TemporaryDirectory(prefix="omega-g4-metadata-chain-") as directory:
        root = Path(directory)
    ...
    with tempfile.TemporaryDirectory(prefix="omega-g4-max-history-") as directory:
        ...
        print(
            "Generation-4 512-pair synthetic history: "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "launch":
        ...
        _self_test()
        print("Generation-4 match orchestrator self-test passed")
    return 0
