#!/usr/bin/env python3
"""Leakage-resistant generation-4 Omega NNUE trainer.

Generation 4 is deliberately decision aligned: every training example belongs
...
The public workflow is ``plan``, ``run``, ``select``, ``run-robustness``,
``offline-evaluate`` and ``self-test``.
...
import validate_king_state_v4_preregistration as prereg_validator
...
PLAN_KIND = "omega-nnue-king-state-v4-training-plan"
CANDIDATE_KIND = "omega-nnue-king-state-v4-candidate"
FAILURE_KIND = "omega-nnue-king-state-v4-candidate-failure"
SELECTION_KIND = "omega-nnue-king-state-v4-validation-selection"
ROBUSTNESS_KIND = "omega-nnue-king-state-v4-robustness"
ROBUSTNESS_FAILURE_KIND = "omega-nnue-king-state-v4-robustness-failure"
OFFLINE_CLAIM_KIND = "omega-nnue-king-state-v4-offline-access-claim"
OFFLINE_REPORT_KIND = "omega-nnue-king-state-v4-offline-report"
PROFILE_KIND = "omega-nnue-king-state-v4-preregistration"
PROFILE_STATUS = "target-blind-generation-4-design-frozen-before-teacher-labels"
...
CANDIDATES = ("G4A", "G4B", "G4C")
TIE_PRIORITY = ("G4B", "G4C", "G4A")
CANDIDATE_RECIPES = {
    "G4A": {"rank": 4, "rankingWeight": 0.25},
    "G4B": {"rank": 4, "rankingWeight": 0.50},
    "G4C": {"rank": 8, "rankingWeight": 0.50},
}
...
DATA_DIR = REPO / "build-msvc" / "data-generation" / "omega-decision-v1"
OUTPUT_DIR = REPO / "build-msvc" / "king-state-v4"
...
OFFLINE_BOOTSTRAP_SEED = 2026072207
...
def _verify_final_freeze_binding(
    ...
    seal = _load_json(seal_path, "generation-4 final-freeze seal")
    expected_fields = {
        "schemaVersion",
        "kind",
        "profileId",
...
def _factor_basis(seed: int, rank: int) -> np.ndarray:
    if rank not in (4, 8):
        raise ValueError("generation-4 rank must be 4 or 8")
...
def _runtime_health(
    ...
    *,
    ...
    if cpp_evaluator is not None:
        with tempfile.TemporaryDirectory(prefix="omega-g4-health-") as directory:
...
def _train_candidate(
    *,
    ...
    if candidate_id not in CANDIDATES:
        raise ValueError(f"unknown generation-4 candidate {candidate_id}")
    ...
    if batch_size != BATCH_SIZE and not synthetic_schedule:
        raise ValueError("generation-4 batch size is frozen at 256")
    if not synthetic_schedule and (epochs, qat_epochs) != (EPOCHS, QAT_EPOCHS):
        raise ValueError("generation-4 epoch schedule is frozen at 48/12")
...
def _validate_profile(path: Path) -> dict[str, Any]:
    profile = _load_json(path, "generation-4 preregistration")
    ...
    if (
        ...
        raise ValueError("wrong generation-4 preregistration")
...
def _prepare_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if audit["rows"] != 16384 or audit["roots"] != 4096:
        raise ValueError("generation-4 corpus is not exactly 4,096 x 4 rows")
    phase_root_totals = {
        phase: len(
            {
                root
    ...
    if any(value != 1024 for value in phase_root_totals.values()):
        raise ValueError("generation-4 phase quotas are not exactly 1,024 roots")
    ...
    if plan_path.exists() or selection_path.exists():
        raise FileExistsError(plan_path if plan_path.exists() else selection_path)
    for candidate in CANDIDATES:
        ...
        for path in (
            expected_bundle,
            _basis_path(candidate, output_dir),
    ...
    for candidate in CANDIDATES:
        ...
        _exclusive_bytes(basis_path, payload)
...
def _verify_plan(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    plan = _load_json(plan_path, "generation-4 training plan")
    ...
    if set(plan) != expected_plan_fields or plan.get("schemaVersion") != SCHEMA_VERSION:
        raise ValueError("generation-4 training-plan field inventory changed")
    if plan.get("kind") != PLAN_KIND or plan.get("profileId") != PROFILE_ID:
        raise ValueError("wrong generation-4 training plan")
...
def _run_worker(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.candidate not in CANDIDATES:
        raise ValueError("unknown generation-4 candidate")
    output_dir = _resolve(args.output_dir)
    ...
    claim = {
        "schemaVersion": SCHEMA_VERSION,
        "kind": (
            "omega-nnue-king-state-v4-robustness-training-claim"
            if robustness
            else "omega-nnue-king-state-v4-training-claim"
        ),
        "profileId": PROFILE_ID,
...
def _run(args: argparse.Namespace) -> None:
    ...
    for candidate in candidates:
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"generation-4 worker {candidate} exited {completed.returncode}"
            )
    ...
    if manifests:
        ...
        for candidate, manifest in manifests.items():
            ...
            if (
                ...
                or manifest.get("i0Validation") != recomputed_i0
                or manifest.get("zeroResidualValidation") != recomputed_zero
            ):
...
def _verify_selection(
    ...
    selection = _load_json(selection_path, "generation-4 selection")
    expected_fields = {
        "schemaVersion",
        "kind",
        "profileId",
    ...
    if set(selection) != expected_fields:
        raise ValueError("generation-4 selected-seal field inventory changed")
    if (
        ...
        raise ValueError("generation-4 selection did not authorize robustness")
    try:
...
def _verify_robustness_training_claim(
    ...
    *,
    ...
    expected = {
        "schemaVersion": SCHEMA_VERSION,
        "kind": "omega-nnue-king-state-v4-robustness-training-claim",
        "profileId": PROFILE_ID,
        "candidateId": candidate,
        "plan": selection["plan"],
        "outputDirectory": str(output_dir),
...
def _verify_robustness(
    ...
    value = _load_json(robustness_path, "generation-4 robustness seal")
    expected_fields = {
        "schemaVersion",
        "kind",
        "createdUtc",
...
def _verify_offline_claim(
    ...
    *,
    ...
    value = _load_json(claim_path, "generation-4 offline access claim")
...
def _verify_offline_report(path: Path) -> dict[str, Any]:
    ...
    value = _load_json(report_path, "generation-4 offline report")
    expected_fields = {
        "schemaVersion",
        "kind",
        "createdUtc",
...
def _offline_evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if report_path.exists():
        ...
        if not report["gate"]["passed"]:
            raise ValueError("generation-4 held-out gate did not pass")
        return report
...
def _self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="omega-g4-trainer-self-test-") as directory:
        root = Path(directory)
        ...
        stored_basis = _load_factor_basis(
            basis_identity, candidate="G4A", rank=4, seed=12345
        )
        ...
        paths = _candidate_paths("G4A", root / "bundles")
        ...
        interrupted_paths = _candidate_paths("G4B", root / "bundles")
        try:
            _publish_bundle(
                interrupted_paths,
                {
        ...
        claim_path = _candidate_claim_path("G4C", root / "bundles")
        _exclusive_json(claim_path, {"candidateId": "G4C"})
        try:
            _exclusive_json(claim_path, {"candidateId": "G4C"})
        except FileExistsError:
            pass
        else:
            raise AssertionError("atomic worker claim admitted two owners")
        claim_path.unlink()
        ...
        _exclusive_json(
            robustness_claim_path,
            {
                ...
                "trainingSeed": _candidate_seed(
                    robustness_profile, "G4A", "robustness-training"
                ),
                "factorSeed": _candidate_seed(
                    robustness_profile, "G4A", "factor-basis"
                ),
        ...
        # resampling without touching any real generation-4 label.
        ...
        if _candidate_bundle(
            "G4A", root / "bundles", robustness=True
        ) == _candidate_bundle("G4A", root / "bundles"):
            raise AssertionError("robustness bundle overlaps primary bundle")
        if _candidate_claim_path(
            "G4A", root / "bundles", robustness=True
        ) == _candidate_claim_path("G4A", root / "bundles"):
