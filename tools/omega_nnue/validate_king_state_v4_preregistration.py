"""Validate the generation-4 Omega NNUE preregistration template or freeze.
...
DEFAULT_PROFILE = (
    REPO
    / "validation"
    / "omega-nnue-king-state-v4-preregistration.template.json"
)
SCHEMA_VERSION = 1
PROFILE_ID = "king-state-v4-omega-decision-v1"
TEMPLATE_KIND = "omega-nnue-king-state-v4-preregistration-template"
FROZEN_KIND = "omega-nnue-king-state-v4-preregistration"
TEMPLATE_STATUS = (
    "draft-not-executable-until-development-grid-and-implementation-"
    "identities-are-sealed"
)
FROZEN_STATUS = "target-blind-generation-4-design-frozen-before-teacher-labels"
GRID_PLACEHOLDER = "<FROM_FROZEN_DEVELOPMENT_GRID>"
CLOSURE_PLACEHOLDER = "<FROM_VERIFIED_G3_CLOSURE>"
...
EXPECTED_NAMESPACES = {
    "dataProfile": "omega-decision-v1",
    "dataRoot": "build-msvc/data-generation/omega-decision-v1",
    "trainingRoot": "build-msvc/king-state-v4",
    "developmentGridRoot": "build-msvc/king-state-v4-development",
    "offlineRoot": "build-msvc/king-state-v4/offline",
    "matchRoot": "build-king-state-v4/matches",
    "candidateBundles": {
        "G4A": "build-msvc/king-state-v4/G4A.bundle",
        "G4B": "build-msvc/king-state-v4/G4B.bundle",
        "G4C": "build-msvc/king-state-v4/G4C.bundle",
    },
    "robustnessBundle": "build-msvc/king-state-v4/robustness.bundle",
    "robustnessClaim": (
        "build-msvc/king-state-v4/.robustness.training.claim.json"
    ),
    "robustnessFailure": "build-msvc/king-state-v4/robustness.failure.json",
    "selectionSeal": (
        "build-msvc/king-state-v4/validation-selection.seal.json"
    ),
    "robustnessSeal": "build-msvc/king-state-v4/robustness.seal.json",
    "preLabelSeal": (
        "build-msvc/data-generation/omega-decision-v1/"
        "prelabel-freeze.seal.json"
    ),
    "finalPreregistration": (
        "validation/omega-nnue-king-state-v4-preregistration.json"
    ),
    "finalFreezeSeal": "validation/omega-nnue-king-state-v4-freeze.seal.json",
    "offlineAccessClaim": (
        "build-msvc/king-state-v4/offline/access-claim.json"
    ),
    "offlineReport": "build-msvc/king-state-v4/offline/report.json",
    "matchStages": {
        "development": "build-king-state-v4/matches/development",
        "equalNode": "build-king-state-v4/matches/equal-node",
        "equalTime": "build-king-state-v4/matches/equal-time",
    },
}

EXPECTED_SEEDS = {
    "source": 2026072201,
    "rootSample": 2026072202,
    "siblingExploration": 2026072203,
    "componentSplit": 2026072204,
    "teacher": 2026072205,
    "training": 2026072206,
    "offlineBootstrap": 2026072207,
    "match": 2026072208,
}
...
EXPECTED_FRESH_DECISION_CORPUS = {
    "rootQuota": {
        "completeRoots": 4096,
        "completeRootsPerPhase": 1024,
        "reserveRootsPerPhase": 256,
        "reserveFraction": 0.2,
        "maximumCandidateRoots": 5120,
        "phasesInFrozenOrder": [
            "opening",
            "middlegame",
            "late",
    ...
    "source": {
        ...
        "rootSelectionUsesSeed": 2026072202,
        "rulesOnlyOpeningPool": {
            "generatorSeed": 2026072201,
            ...
            "selectedOpeningsPerPhaseAndSide": 800,
...
EXPECTED_POST_SELECTION = {
    "robustness": {
        ...
        "outputs": {
            "bundle": "build-msvc/king-state-v4/robustness.bundle",
            "claim": (
                "build-msvc/king-state-v4/.robustness.training.claim.json"
            ),
            "failure": "build-msvc/king-state-v4/robustness.failure.json",
            "seal": "build-msvc/king-state-v4/robustness.seal.json",
        },
        "publication": (
    ...
    "oneTimeHeldOut": {
        ...
        "accessClaim": "build-msvc/king-state-v4/offline/access-claim.json",
        "report": "build-msvc/king-state-v4/offline/report.json",
        "resumePolicy": (
...
EXPECTED_TEMPLATE_PLACEHOLDERS = {
    "generation3Closure.bytes": CLOSURE_PLACEHOLDER,
    "generation3Closure.sha256": CLOSURE_PLACEHOLDER,
...
EXPECTED_FINAL_IDENTITY_PATHS = {
    ...
    "cppStaticAndNnueEvaluator": (
        "tools/omega_nnue/frozen_runtime/king-state-v4/evaluator/omega_nnue.exe"
    ),
    "sourceMatchHarnessAssembly": (
        "tools/omega_nnue/frozen_runtime/king-state-v4/omegamatch/"
        "OmegaMatch.dll"
    ),
    "sourceOpeningBuilderSource": (
        "tools/omega_nnue/king_state_generation4_source.py"
    ),
    "finalFreezeBuilderSource": (
        "tools/omega_nnue/king_state_generation4_freeze.py"
    ),
    "sourceRootPool": (
    ...
    "trainerSource": "tools/omega_nnue/king_state_train_generation4.py",
    "baseTrainerSource": "tools/omega_nnue/train.py",
    "pythonRuntimeManifest": (
        "validation/omega-nnue-king-state-v4-python-runtime.json"
    ),
    "pythonRuntimeToolSource": (
        "tools/omega_nnue/king_state_generation4_runtime.py"
    ),
    "priorProjectionSource": (
    ...
    "dotnetRuntimeHostExecutable": (
        "tools/omega_nnue/frozen_runtime/king-state-v4/dotnet-runtime/"
        "dotnet.exe"
    ),
    "dotnetRuntimeManifest": (
        "tools/omega_nnue/frozen_runtime/king-state-v4/"
    ...
    "matchProtocol": (
        "validation/omega-nnue-king-state-v4-match-protocol.json"
    ),
    "matchProtocolTool": (
        "tools/omega_nnue/king_state_match_protocol_generation4.py"
    ),
    "matchReadinessTool": (
        "tools/omega_nnue/king_state_match_readiness_generation4.py"
    ),
    "matchOrchestrator": (
...
EXPECTED_TEMPLATE_PROFILE_TYPE_INVENTORY = {
    "nodes": 681,
    "scalarLeaves": 569,
    "numericOrBooleanLeaves": 277,
    "sha256": "dbdfbc3285da439dd938a9be383dfc2e0e26d4ebd178e44add1797ae019136a6",
}
EXPECTED_FROZEN_PROFILE_TYPE_INVENTORY = {
    "nodesExcludingForbiddenCatalogs": 679,
    "sha256": "bc482f77f34f7ac54d2d6ff9a4797445a56cc3e2302512facfec7791880b4f7c",
}
...
def _verify_runtime_contract_binding() -> None:
    expected = (
        REPO / "tools/omega_nnue/king_state_generation4_runtime.py"
    ).resolve()
    if runtime_contract is not _IMPORTED_RUNTIME_CONTRACT:
        raise ValueError("Generation-4 Python runtime module was substituted in memory")
    if sys.modules.get("king_state_generation4_runtime") is not _IMPORTED_RUNTIME_CONTRACT:
        raise ValueError("Generation-4 Python runtime import slot was substituted")
...
def _validate_namespaces(value: Any) -> None:
    ...
    _expect(namespaces, EXPECTED_NAMESPACES, "fresh generation-4 namespaces")
    paths: list[str] = []
    ...
    if len(paths) != len(set(paths)):
        raise ValueError("generation-4 namespace paths are not unique")
    if any("king-state-v3" in path or "deep-hce-v4" in path for path in paths):
        raise ValueError("generation-4 namespace overlaps generation 3")
...
def _validate_seeds(value: Any) -> None:
    ...
    if set(seeds) != expected_fields:
        raise ValueError("generation-4 seed field inventory changed")
    ...
    if len(numeric) != len(set(numeric)):
        raise ValueError("generation-4 seeds are not unique")
    _expect(
...
def _validate_corpus(value: Any) -> None:
    ...
    quota = _mapping(corpus.get("rootQuota"), "root quota")
    expected_quota = {
        "completeRoots": 4096,
        "completeRootsPerPhase": 1024,
        "reserveRootsPerPhase": 256,
    ...
    _expect(dict(quota), expected_quota, "decision root quota")
    source = _mapping(corpus.get("source"), "decision source")
    for key, expected in {
        "freshRunRequired": True,
        "hceOnly": True,
        "requiredRunSeed": 2026072201,
        ...
        _expect(source.get(key), expected, f"decision source {key}")
    _expect(
        source.get("rootSelectionMayReadTargetsOrScores"),
        False,
        "target-opaque root selection",
    )
    _expect(source.get("rootSelectionUsesSeed"), 2026072202, "root seed")
    _expect(
        dict(_mapping(source.get("rulesOnlyOpeningPool"), "source opening pool")),
        {
            "generatorSeed": 2026072201,
            "trajectoryPairs": 8192,
            "independentTrajectoriesPerPair": 2,
        ...
        "source rules-only opening-pool recipe",
    )
    _expect(
        dict(_mapping(source.get("choiceProbe"), "source choice probe")),
        {
            "colorSwappedGamesPerOpening": 2,
            "sameFrozenHceBinaryForBothEngineIds": True,
            "fixedNodesPerMove": 2000,
        ...
        "source choice-probe recipe",
    )
    engine = _mapping(corpus.get("teacherEngine"), "teacher engine")
    for key, expected in {
        "UseOmegaNNUE": False,
        "OmegaNNUEFile": "<empty>",
        "Threads": 1,
        ...
        _expect(engine.get(key), expected, f"teacher {key}")
    legal = _mapping(corpus.get("legalChildExpansion"), "legal expansion")
    _expect(
        legal.get("enumerateEveryLegalChildExactlyOnce"),
        True,
        "complete child expansion",
    )
    shallow = _mapping(corpus.get("shallowScreen"), "shallow screen")
    _expect(shallow.get("fixedNodesPerChild"), 2000, "shallow nodes")
    _expect(shallow.get("preLabelSealRequired"), True, "shallow pre-label seal")
    deep_selection = _mapping(corpus.get("deepSelection"), "deep selection")
    _expect(
        deep_selection.get("selectedChildrenPerRoot"),
        4,
        "selected children per root",
    )
    _expect(deep_selection.get("seriousCandidates"), 3, "serious candidates")
    _expect(
        deep_selection.get("hardNegativeRule"),
        "one deterministic sibling sampled from shallow ranks 4 through 12 "
        "inclusive using siblingExploration seed; if fewer than twelve exist "
        "use every available rank in that interval; if the interval is empty "
        "reject the root",
        "hard-negative rank interval",
    )
    _expect(
        deep_selection.get("selectionUsesDeepTargets"),
        False,
        "target-blind child selection",
    )
    deep = _mapping(corpus.get("deepTeacher"), "deep teacher")
    _expect(deep.get("fixedNodesPerSelectedChild"), 50000, "deep nodes")
    _expect(
        deep.get("wholeRootRejectedIfAnySelectedChildIsMissingOrInvalid"),
        True,
        "whole-root rejection",
    )
    _expect(deep.get("preLabelSealRequired"), True, "deep pre-label seal")
    runtime = _mapping(corpus.get("teacherSearchRuntime"), "teacher runtime")
    _expect(
        dict(runtime),
        {
            "workers": 4,
    ...
    artifacts = list(_sequence(corpus.get("artifacts"), "corpus artifacts"))
    _expect(
        artifacts,
        [
            "roots.jsonl",
    ...
    split = _mapping(corpus.get("split"), "component split")
    for key, expected in {
        "seed": 2026072204,
        "namesInFrozenOrder": ["train", "validation", "heldOut"],
        "trainPercent": 80,
        ...
        _expect(split.get(key), expected, f"split {key}")
    policy = _mapping(corpus.get("artifactPolicy"), "artifact policy")
    for key in (
        "everyArtifactHasANoClobberManifest",
        "resultLedgersAreAppendOnlyAndFsynced",
        "publishedDatasetsUseAtomicNonReplacingRename",
        "manifestsPinPathBytesSha256RowCountAndProducerIdentity",
        "finalStageSealIsThePublicationCommitPoint",
    ):
        _expect(policy.get(key), True, f"artifact policy {key}")
...
def _validate_prelabel_gates(value: Any) -> None:
    ...
    expected = {
        ...
        "priorSourceInputSha256Collisions": 0,
    ...
    _expect(
        ...
        "pre-label target-opaque input inventory",
    )
...
def _validate_candidates(value: Any) -> None:
    ...
    g4b = _mapping(candidates[1], "G4B")
    update = _mapping(g4b.get("conditionedUpdate"), "G4B update")
    _expect(
        update.get("formula"),
...
def _validate_grid(
    ...
    *,
    ...
    _expect(
        grid.get("mayUseGeneration4Targets"),
        False,
        "development grid generation-4 target access",
    )
    _expect(
        grid.get("mustBeSealedBeforeGeneration4PrimaryTraining"),
        True,
        "grid seal timing",
    )
    settings = dict(
...
def validate_profile(
    ...
    *,
    ...
    if mode == "template":
        ...
        closure = _mapping(profile.get("generation3Closure"), "G3 closure")
        _expect(
            closure.get("path"),
            "validation/omega-nnue-king-state-v3-closure.json",
            "G3 closure path",
        )
    else:
        if placeholders:
        ...
        _validate_identity(
            profile.get("generation3Closure"),
            "generation-3 closure",
            verify_file=verify_external,
        )

    boundary = _mapping(profile.get("informationBoundary"), "boundary")
    for key in (
        "generation3HeldOutTargetsAvailableToThisDesign",
        "generation3HeldOutMetricsAvailableToThisDesign",
        "generation3MatchResultsAvailableToThisDesign",
        "generation4TeacherTargetsAvailableAtFinalFreeze",
        "generation4ValidationTargetsAvailableAtFinalFreeze",
        "generation4HeldOutTargetsAvailableAtFinalFreeze",
        "heldOutTargetsMaySelectRecipe",
        "matchResultsMaySelectRecipe",
    ):
        _expect(boundary.get(key), False, f"information boundary {key}")
    immutability = _mapping(profile.get("immutability"), "immutability")
    _expect(
        immutability.get("dataNamespacesSeedsQuotasAndAbortGatesMayChangeAfterTemplate"),
        False,
        "data freeze",
    )
    _expect(
        immutability.get("candidateArchitectureAndRankMatrixMayChangeAfterTemplate"),
        False,
        "candidate matrix freeze",
    )
    _expect(
        immutability.get("deploymentHealthThresholdsMayChangeAfterTemplate"),
        False,
        "health threshold freeze",
    )
    _expect(
        immutability.get("activationPenaltyStrengthsMustComeOnlyFromFrozenDevelopmentGrid"),
        True,
        "activation source freeze",
    )
    ...
    _expect(
        initializer.get("generation3CandidateWeightsMayInitializeGeneration4"),
        False,
        "failed G3 candidates cannot initialize G4",
    )
    migration = _mapping(
...
def _self_test(profile: Mapping[str, Any]) -> None:
    ...
    synthetic_frozen["generation3Closure"]["bytes"] = 1
    synthetic_frozen["generation3Closure"]["sha256"] = "0" * 64
    ...
    must_fail(
        lambda value: value["seeds"].__setitem__("training", 2026072205),
        "reused seed",
    )
    must_fail(
        lambda value: value["seeds"].__setitem__("match", 2026072208.0),
    ...
    must_fail(
        lambda value: value["finalFreezeIdentities"]["matchCoreSource"].__setitem__(
            "path", "tools/omega_nnue/king_state_matches_generation4.py"
        ),
    ...
    must_fail(
        lambda value: value["finalFreezeIdentities"][
            "dotnetRuntimeHostExecutable"
        ].__setitem__("path", "tools/omega_nnue/frozen_runtime/king-state-v4/engine/senpai.exe"),
        "changed frozen .NET host path",
    )
    must_fail(
        lambda value: value["finalFreezeIdentities"]["dotnetRuntimeManifest"].__setitem__(
            "path", "validation/omega-nnue-king-state-v4-python-runtime.json"
        ),
    ...
    must_fail(
        lambda value: value["finalFreezeIdentities"][
            "dotnetRuntimeToolSource"
        ].__setitem__("path", "tools/omega_nnue/king_state_generation4_runtime.py"),
        "changed frozen .NET runtime tool path",
    )
    must_fail(
        lambda value: value["finalFreezeIdentities"]["matchProtocol"].__setitem__(
            "path", "validation/omega-nnue-king-state-v4-preregistration.template.json"
        ),
    ...
    must_fail(
        lambda value: value["finalFreezeIdentities"]["matchReadinessTool"].__setitem__(
            "path", "tools/omega_nnue/king_state_match_protocol_generation4.py"
        ),
        "changed frozen match-readiness tool path",
    )
    must_fail(
        lambda value: value["finalFreezeIdentities"]["matchOrchestrator"].__setitem__(
            "path", "tools/omega_nnue/king_state_match_readiness_generation4.py"
        ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "validate":
        ...
        print(
            f"Validated generation-4 {args.mode} profile: {profile_path} "
            f"({profile_path.stat().st_size} bytes, {_sha256(profile_path)})"
        )
        return 0
    if args.command == "self-test":
        _self_test(profile)
        print("Generation-4 preregistration validator self-tests passed.")
        return 0
